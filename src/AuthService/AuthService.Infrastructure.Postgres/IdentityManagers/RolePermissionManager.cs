...
public class RolePermissionManager
{
    ...
    public RolePermissionManager(PostgresDbContext postgresContext)
    {
        ArgumentNullException.ThrowIfNull(postgresContext);
        _db = postgresContext;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        HashSet<Guid> alreadyLinked = new(alreadyLinkedPermissionIds);

        List<RolePermission> toAdd = new();
        foreach (Permission p in existingPermissions)
        {
            ...
            RolePermission link = new() { RoleId = roleId, PermissionId = p.Id };
            toAdd.Add(link);
        }
